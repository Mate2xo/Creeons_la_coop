...
ActiveAdmin.register StaticSlot do
  permit_params :week_day, :hours, :hour, :minute, :week_type, static_slot_ids: []
  ...
  index do
    ...
    column(:hours) { |resource| resource.hours.strftime('%Hh%M') }
    column :week_type
    actions
  end
  ...
  show do
    attributes_table_for resource do
      ...
      row(:hours) { resource.hours.strftime('%Hh%M') }
      row(:week_type)
      row(:created_at)
      row(:updated_at)
    end
  ...
  controller do
    def create
      ...
      permitted_params.merge!({ hours: DateTime.new(2020, 1, 1, permitted_params[:hour].to_i,
